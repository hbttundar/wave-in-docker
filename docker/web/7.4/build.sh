...
apt-get update &&
  ...
  echo "deb http://ppa.launchpad.net/ondrej/php/ubuntu focal  main" >/etc/apt/sources.list.d/ppa_ondrej_php.list  #use hirsute for ubuntu 21.04
...
if [ "$DEFAULT_WEB_SERVER" = "apache2" ]; then
  ...
  cp /tmp/wave/startup.sh /usr/bin/startup.sh
